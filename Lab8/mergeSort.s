...
mergeSort:
	...
	shr rdx, 1 		; divide by 2 to get midpoint
	push rdx 		
	call mergeSort 		; repeat on first half
	pop rsi 		; mid = start
	pop rdx 		; end = start
	inc rsi 		; mid + 1 
	push rdx 		
	call mergeSort 		; repeat on second half
	pop rdx 		; replace end in right param
	pop rsi 		; replace start in left param
	mov rcx, rdx 		; copy end to another register
	add rdx, rsi		; start + end
	shr rdx, 1 		; divide  by 2 to get midpoint
	call merge 		; merge
